# Lifeline Documentation

This directory contains the repository’s organized documentation surface after the Phase 2.5 cleanup.

## Sections
- [guides/](guides)
  - current how-to and usage-oriented documents
- [reference/](reference)
  - reference material, supporting artifacts, and retained non-root technical docs
- [archive/](archive)
  - stale or superseded but historically useful documents
- [issues/](issues)
  - issue-centered project history and phase artifacts

## Issue-Centered History
- [DB migration prep](issues/db-migration-prep)
  - [Phase 1](issues/db-migration-prep/phase-1)
  - [Phase 2](issues/db-migration-prep/phase-2)
- [Repo hygiene](issues/repo-hygiene)
  - [Phase 2.5](issues/repo-hygiene/phase-25)
- [Repo history notes](issues/repo-history)

## Recommended Starting Points
- Project overview: [../README.md](../README.md)
- Quick start: [guides/QUICK_START.md](guides/QUICK_START.md)
- Testing reference: [reference/TESTING_CHECKLIST.md](reference/TESTING_CHECKLIST.md)
- Historical integration material: [archive/](archive)
