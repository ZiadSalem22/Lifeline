...
7. Runs `docker compose` with [compose.production.yaml](../../compose.production.yaml) and `/opt/lifeline/shared/.env.production` in two stages: PostgreSQL plus app first, then a targeted `lifeline-mcp` force-recreate after app health is restored.
...
9. Verifies app, MCP, and Postgres container health plus internal and public app runtime checks, including the public app info endpoint, MCP loopback publication on `127.0.0.1:3030`, and the MCP-to-backend internal adapter path.
