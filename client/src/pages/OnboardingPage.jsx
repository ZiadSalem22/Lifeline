...
export default function OnboardingPage({ user, currentUser, guestMode, onCompleted }) {
  ...
  const { checkedIdentity } = useAuthContext();

  // Wait until identity is firmly checked before redirecting
  if (!checkedIdentity && !guestMode) {
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            <input value={country} onChange={e => {
              setCountry(e.target.value); // attempt to set sensible default start day
