            ...
            <div key={opt.key} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              {mode === opt.key && (
                <div style={{ fontSize: '0.74rem', color: 'var(--color-text)', padding: '6px 10px', marginLeft: 36, borderLeft: '3px solid rgba(0,0,0,0.06)', lineHeight: 1.25 }}>
                  ...
                  {opt.key === 'dateRange' && (
                    <div><strong style={{fontWeight:600}}>Date Range:</strong> Creates one logical task that appears on every day between the start and end dates. Marking this task complete will mark the entire date range as completed and it will no longer appear on remaining days.</div>
                  )}
