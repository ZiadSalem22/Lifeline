...
*   **Entry Point:** `frontend/index.html` (mounts to `<div id="root"></div>`) which loads `frontend/src/main.jsx`. `frontend/src/main.jsx` renders the main `App` component.
*   **Core Component:** `frontend/src/App.jsx` - Manages global application state (todos, tags, theme, selected dates), handles API interactions, and orchestrates the rendering of major UI components.
*   **API Integration:** `frontend/src/api.js` - Centralizes all communication with the backend API. It provides asynchronous functions for fetching, creating, updating, and deleting todos and tags.
*   **Components:** Modular React components (e.g., `Sidebar.jsx`, `Settings.jsx`, `CosmicBackground.jsx`, `TaskCard` within `App.jsx`).
*   **Styling:** Primarily uses global CSS imports (`theme.css`, `responsive.css`) and inline styles within JSX. `framer-motion` is used for animations.
...
### 2.2 Frontend Commands (from `frontend/package.json`)
...
### 3.1 Data Flow
*   **Frontend to Backend:** User actions -> `frontend/src/api.js` functions -> HTTP requests to `http://localhost:3000/api/todos` or `http://localhost:3000/api/tags`.
...
*   `frontend/src/App.jsx`: Main frontend component, state management.
*   `frontend/src/api.js`: Centralized frontend API calls.
*   `frontend/src/theme.css`, `frontend/src/responsive.css`: Global frontend styles.
